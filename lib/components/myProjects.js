...
class MyProjects extends Component {
	...
	_handleUpdateForm = (proj) => {
		this.setState({
			...
			model: {
				...proj, Date_: new Date(proj.Date_)
			}
		})
	}
		  ...
		  buttons: [
			{
			  ...
			  onClick: () => {
				const userLog = JSON.parse(sessionStorage.userInfo);
				this.props.DeleteProject(id, userLog.token);
			  }
			},
			{
			  label: 'No',
	...
	_save = async (id, token) => {
		const model = {
			...
			Title: '',
			Url: '',
			Date_: moment(this.state.model.Date_).format(apiconfig.FORMAT_DATE)
		}
	...
	_handleDayClick= (day, { selected }) =>{
		this.setState({
			model:{...this.state.model, Date_: selected ? new Date() : day}
		});}
  render() {
	return (
