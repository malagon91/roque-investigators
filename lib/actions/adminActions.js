...
export function deleteUser(id, tokenStr){
  return dispatch =>{
    ...
    axios({
      ...
      toastr.success("Se guardo la informacion con exito","Informacion",optionsTo)
      dispatch(loadInvestigators());
    }).catch (error => {
      dispatch({type: FINISH_LOADING_PANEL})
      toastr.error("Error al procesar la informacion, intenta de nuevo por favor","Error", optionsTo)
    });
  }
}
...
export function resetPassword(id, password,tokenStr ){
  return dispatch =>{
    dispatch({type: INIT_LOADING_PANEL})
    const url = `${apiconfig.PREFIX}${apiconfig.RESET_PASSWORD}`
    const model = {
      id: id,
      password: password 
    }
    axios({
      ...
      dispatch({type: FINISH_LOADING_PANEL})
      toastr.success("Se actualizo la informacion con exito","Inofrmacion",optionsTo)
    }).catch(error =>{
      dispatch({type: FINISH_LOADING_PANEL})
      toastr.error("Error al actualizar la informacion","Error",optionsTo)
    })
  }
}
...
export function updatePrivileges(id,newAdmin,tokenStr){
  return dispatch =>{
    dispatch({type: INIT_LOADING_PANEL})
    const url = `${apiconfig.PREFIX}${apiconfig.UPDATE_PRIVILEGES}`
    const model = {
      id: id,
      isAdmin: newAdmin 
    }
