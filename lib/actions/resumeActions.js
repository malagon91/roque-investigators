...
import {
	...
	BEGIN_AJAX_CALL,AJAX_CALL_ERROR,
...
export function updateResume(resume){
	return dispatch =>{
		dispatch({type: BEGIN_AJAX_CALL})
		delete resume.existImage
		axios({
			method:'PUT',
			data: resume,
			url: apiconfig.USER,
			...
			toastr.info("Se actualizo la informacion con exito","Informacion", optionsTo);
