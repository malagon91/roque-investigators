...
export function changePassword(id, old, _new, tokenStr,router){
	return dispatch =>{
		dispatch({type: INIT_LOAD_FORM})
		const url = `${apiconfig.PREFIX}${apiconfig.CHANGE_PASSWORD}`;
		const model = {
			_id: id,
			_old: old,
			_new: _new
		...
		axios({
			...
			if (result.data.success){
				toastr.success("Se actualizo el password con exito","Informacion", optionsTo);
				dispatch({type:END_LOAD_FORM});
				router.push('/resume');
			}else{
				toastr.warning(`Error al actualizar: ${result.data.message}`,`Error`, optionsTo);
				dispatch({type:END_LOAD_FORM});
			}
		}).catch(error =>{
			toastr.error(`Error al actualizar, intentalo mas tarde`,`Error`, optionsTo);
			dispatch({type:END_LOAD_FORM});
		})
	}
}
